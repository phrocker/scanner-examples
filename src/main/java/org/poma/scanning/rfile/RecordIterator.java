...
import org.poma.scanning.mr.BulkIteratorEnvironment;
import org.poma.scanning.mr.split.FileRangeSplit;
import org.poma.scanning.mr.split.RangeSplit;
import org.poma.scanning.rfile.split.TabletSplitSplit;
...
public class RecordIterator extends RangeSplit implements SortedKeyValueIterator<Key,Value>, Closeable {
    ...
    protected SortedKeyValueIterator<Key,Value> buildTopIterators(SortedKeyValueIterator<Key,Value> topIter, Configuration conf) throws ClassNotFoundException,
        ...
        List<BulkInputFormat.AccumuloIterator> iterators = BulkInputFormat.getIterators(conf);
        List<BulkInputFormat.AccumuloIteratorOption> options = BulkInputFormat.getIteratorOptions(conf);
        
        Map<String,IteratorSetting> scanIterators = new HashMap<>();
        for (BulkInputFormat.AccumuloIterator iterator : iterators) {
        ...
        for (BulkInputFormat.AccumuloIteratorOption option : options) {
