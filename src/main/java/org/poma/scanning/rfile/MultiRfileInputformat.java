...
import org.poma.scanning.mr.RFileInputFormat;
import org.poma.scanning.mr.split.FileRangeSplit;
import org.poma.scanning.rfile.split.RfileSplit;
...
public class MultiRfileInputformat extends RFileInputFormat {
    ...
    private static LoadingCache<Range,Set<Pair<String,Set<String>>>> locationMap = null;
    ...
    public static List<InputSplit> computeSplitPoints(Connector conn, Configuration conf, String tableName, List<Range> ranges) throws TableNotFoundException,
        ...
        for (Range range : ranges) {
            ...
            Set<Pair<String,Set<String>>> metadataEntries;
            try {
                if (null == locationMap) {
            ...
            for (Pair<String,Set<String>> entry : metadataEntries) {
                String location = entry.getFirst();
                Set<String> fileLocations = entry.getSecond();
