...
import org.poma.scanning.rfile.RfileResource;
import org.poma.scanning.stats.ScanSessionStats;
import org.poma.scanning.manager.RangeDefinition;
import org.poma.scanning.manager.ResourceQueue;
import org.poma.scanning.manager.ShutdownEvent;
...
public class Scan implements Callable<Scan> {
    ...
    protected BlockingQueue<Map.Entry<Key, Value>> results;
    ...
    public Scan(String localTableName, Set<Authorizations> localAuths, ScannerChunk chunk, ResourceQueue delegatorReference,
                Class<? extends AccumuloResource> delegatedResourceInitializer, BlockingQueue<Map.Entry<Key,Value>> results, ExecutorService callingService) {
        myScan = chunk;
    ...
    public Scan call() throws Exception {
        try {
            ...
            do {
                if (null != myStats)
                    myStats.getTimer(ScanSessionStats.TIMERS.SCANNER_START).resume();
                ...
                if (lastSeenKey == null || (currentRange != null && currentRange.getEndKey() != null && lastSeenKey.compareTo(currentRange.getEndKey()) >= 0)) {
                    ...
                    if (null == currentRange) {
                        ...
                        if (log.isTraceEnabled())
                            log.trace("Leaving");
                        if (null != myStats)
                            myStats.getTimer(ScanSessionStats.TIMERS.SCANNER_START).suspend();
                        return this;
                    }
                ...
                Iterator<Map.Entry<Key,Value>> iter = delegatedResource.iterator();
                ...
                Map.Entry<Key,Value> myEntry = null;
                if (null != myStats)
                    myStats.getTimer(ScanSessionStats.TIMERS.SCANNER_ITERATE).resume();
                while (iter.hasNext()) {
                ...
                if (null != myStats)
                    myStats.getTimer(ScanSessionStats.TIMERS.SCANNER_ITERATE).suspend();
                ...
                ThriftScanner.ScanTimedOutException e)
